...
#!/bin/bash
echo "🚀 Starting Windows VM (NO EXIT MODE)"

# download iso (NEVER EXIT)
if [ ! -f /iso/os.iso ]; then
  echo "📥 Downloading Windows ISO (safe)..."
  wget --continue "$ISO_URL" -O /iso/os.iso || echo "ISO download skipped"
fi

# create disk
[ ! -f /data/disk.qcow2 ] && qemu-img create -f qcow2 /data/disk.qcow2 60G
